...
const MAX_DETAIL_FETCHES_PER_SOURCE=6;
...
function unique(items){ const seen=new Set(); return items.filter(item=>{const key=`${item.type}|${item.url}`; if(seen.has(key)) return false; seen.add(key); return true;}); }
...
export function directSources(){
  ...
  const pages=envUrls("DIRECT_SOURCE_URLS").filter(validHttpUrl).filter(pageAllowed).map((url,i)=>({id:`env-page-${i+1}`,label:new URL(url).hostname.replace(/^www\./,""),type:"page",url,enabled:true,maxDetails:6,detailPathPattern:"(property|rent|let|details)"}));
...
function candidateFromLd(node,source,index,method="direct_page_structured_data"){
  ...
  return {id:`${source.id}-ld-${index}`,title,url,snippet:detail.filter(Boolean).join(". "),date:node.dateModified||node.datePublished||null,image:resolveUrl(imageFromLd(node.image),source.url),discoveryProviderLabel:source.label,verificationMethod:method};
}

function parseXml(text,source){
  const entries=[...text.matchAll(/<(item|entry)\b[^>]*>([\s\S]*?)<\/\1>/gi)];
  return entries.slice(0,MAX_ITEMS_PER_SOURCE).map((m,i)=>{const block=m[2];const enclosure=block.match(/<enclosure\b[^>]*url=["']([^"']+)["']/i);return{id:`${source.id}-${i}`,title:tag(block,"title"),url:resolveUrl(atomLink(block),source.url),snippet:tag(block,"description")||tag(block,"summary")||tag(block,"content"),date:tag(block,"pubDate")||tag(block,"published")||tag(block,"updated")||null,image:resolveUrl(enclosure?.[1],source.url),discoveryProviderLabel:source.label,verificationMethod:"direct_feed"};}).filter(x=>x.url&&x.title);
}
function parseJson(data,source){
  ...
  return rows.slice(0,MAX_ITEMS_PER_SOURCE).map((x,i)=>{const photos=Array.isArray(x?.photos)?x.photos:[];const image=first(x?.image,x?.imageUrl,x?.image_url,x?.photo,typeof photos[0]==="string"?photos[0]:photos[0]?.url);return{id:`${source.id}-${i}-${x?.id||""}`,title:first(x?.title,x?.name,x?.address,x?.display_address)||"Rental listing",url:resolveUrl(first(x?.url,x?.link,x?.source_url,x?.listing_url),source.url),snippet:first(x?.description,x?.snippet,x?.summary,x?.details)||"",date:first(x?.date,x?.published_at,x?.updated_at,x?.created_at),image:resolveUrl(image,source.url),discoveryProviderLabel:source.label,verificationMethod:"direct_feed"};}).filter(x=>x.url);
}
...
function detailLinks(html,source){
  const pattern=source.detailPathPattern?new RegExp(source.detailPathPattern,"i"):/(property|to-rent|to-let|letting|rentals|details)/i;
  const out=[]; const seen=new Set(); const rx=/<a\b[^>]*href=["']([^"'#]+)["'][^>]*>([\s\S]*?)<\/a>/gi; let m;
  while((m=rx.exec(html))&&out.length<40){
    const href=resolveUrl(m[1],source.url); if(!href||!sameHost(href,source.url)||href===source.url||seen.has(href))continue;
    const u=new URL(href); if(!pattern.test(`${u.pathname}${u.search}`))continue;
    const text=stripHtml(m[2]); if(text.length<3)continue;
    seen.add(href); out.push({url:href,anchorText:text});
  }
  return out;
}
async function fetchText(url,accept="text/html,application/xhtml+xml;q=0.9,*/*;q=0.5"){
  const response=await fetch(url,{headers:{Accept:accept,"User-Agent":"RentalFinder26/2.0 (+rental discovery; respectful polling)"},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
...
function detailCandidate(html,url,source,index){
  ...
  if(structured.length){
    const exact=structured.find(x=>x.url===url)||structured[0];
    return {...exact,id:`${source.id}-detail-${index}`,url,verificationMethod:"direct_property_detail",discoveryProviderLabel:source.label};
  }
  const title=titleFromHtml(html)||"Rental listing";
  const body=stripHtml(html).slice(0,30000);
  if(!/(rent|let|pcm|bed|apartment|flat|house|studio)/i.test(`${title} ${body}`))return null;
  return {id:`${source.id}-detail-${index}`,title,url,snippet:body,image:imageFromHtml(html,url),date:null,discoveryProviderLabel:source.label,verificationMethod:"direct_property_detail"};
}
...
function statusFromError(source,err,started){
  ...
  return {id:source.id,label:source.label,url:source.url,status:kind,items:0,detailPagesChecked:0,error:message,elapsedMs:Date.now()-started,checkedAt:new Date().toISOString()};
}
async function fetchPageSource(source){
  const started=Date.now();
  try{
    const {text}=await fetchText(source.url);
    const indexStructured=jsonLdBlocks(text).map((x,i)=>candidateFromLd(x,source,i,"direct_page_structured_data")).filter(Boolean);
    const links=detailLinks(text,source).slice(0,Math.min(Number(source.maxDetails)||6,MAX_DETAIL_FETCHES_PER_SOURCE));
    const detailResults=await mapLimit(links,DETAIL_CONCURRENCY,async(link,i)=>{const {text:detailHtml}=await fetchText(link.url);return detailCandidate(detailHtml,link.url,source,i);});
    const details=detailResults.filter(x=>x&&!x.__error);
    const detailErrors=detailResults.filter(x=>x?.__error).length;
    const items=unique([...details,...indexStructured].filter(Boolean)).slice(0,MAX_ITEMS_PER_SOURCE);
    return {items,status:{id:source.id,label:source.label,url:source.url,status:items.length?"working":"empty",items:items.length,detailPagesChecked:links.length-detailErrors,detailPagesFailed:detailErrors,error:null,elapsedMs:Date.now()-started,checkedAt:new Date().toISOString()}};
...
async function fetchFeedSource(source){
  ...
  try{
    ...
    let items;
    if(type.includes("json")||/^[\s\r\n]*[\[{]/.test(text)){try{items=parseJson(JSON.parse(text),source);}catch{throw new Error("Invalid JSON feed");}}
    else items=parseXml(text,source);
    return {items,status:{id:source.id,label:source.label,url:source.url,status:items.length?"working":"empty",items:items.length,detailPagesChecked:0,error:null,elapsedMs:Date.now()-started,checkedAt:new Date().toISOString()}};
  }catch(err){return {items:[],status:statusFromError(source,err,started)};}
}
...
export async function directSourceSearch(){
  ...
  if(!sources.length)throw Object.assign(new Error("No rental sources are configured."),{code:"SEARCH_NOT_CONFIGURED"});
  ...
  const sourceStatuses=settled.map(x=>x.status);
