...
function pageAllowed(url){ const allowed=new Set(envUrls("DIRECT_SOURCE_ALLOWED_HOSTS").map(x=>x.toLowerCase())); try{const host=new URL(url).hostname.toLowerCase();return allowed.has(host)||allowed.has(host.replace(/^www\./,""));}catch{return false;} }
...
function decodeEntities(value=""){return String(value).replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;|&apos;/g,"'").replace(/&nbsp;/g," ").replace(/&#xA3;|&pound;/gi,"£").replace(/&#(\d+);/g,(_,n)=>String.fromCharCode(Number(n)));}
...
function metaContent(html,key){const escaped=key.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");const rxs=[new RegExp(`<meta\\b[^>]*(?:property|name)=["']${escaped}["'][^>]*content=["']([^"']+)["'][^>]*>`,`i`),new RegExp(`<meta\\b[^>]*content=["']([^"']+)["'][^>]*(?:property|name)=["']${escaped}["'][^>]*>`,`i`)];for(const rx of rxs){const m=html.match(rx);if(m)return decodeEntities(m[1]);}return null;}
...
function flattenLd(value,out=[]){if(Array.isArray(value)){value.forEach(x=>flattenLd(x,out));return out;}if(!value||typeof value!=="object")return out;out.push(value);if(Array.isArray(value["@graph"]))value["@graph"].forEach(x=>flattenLd(x,out));if(Array.isArray(value.itemListElement))value.itemListElement.forEach(x=>flattenLd(x?.item||x,out));return out;}
function jsonLdBlocks(html){const blocks=[...html.matchAll(/<script\b[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)];const nodes=[];for(const block of blocks){try{flattenLd(JSON.parse(block[1].trim()),nodes);}catch{}}return nodes;}
...
function candidateFromLd(node,source,index,method="direct_page_structured_data"){
  ...
  const price=offer?.price??node.price;const currency=offer?.priceCurrency||node.priceCurrency;if(price&&(!currency||String(currency).toUpperCase()==="GBP"))detail.push(`£${price} pcm`);
...
function detailLinks(html,source,baseUrl=source.url){
  const pattern=detailPattern(source),out=[],seen=new Set(),rx=/<a\b[^>]*href=["']([^"'#]+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m;
  while((m=rx.exec(html))&&out.length<200){const href=resolveUrl(m[1],baseUrl);if(!href||!sameHost(href,source.url)||canonicalUrl(href)===canonicalUrl(source.url)||seen.has(canonicalUrl(href)))continue;const u=new URL(href);if(!pattern.test(`${u.pathname}${u.search}`))continue;const text=stripHtml(m[2]);if(text.length<3)continue;seen.add(canonicalUrl(href));out.push({url:canonicalUrl(href),anchorText:text});}
  return out;
}
function paginationLinks(html,source,baseUrl=source.url){
  const out=[],seen=new Set(),rx=/<a\b[^>]*href=["']([^"'#]+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m;
  while((m=rx.exec(html))&&out.length<12){const href=resolveUrl(m[1],baseUrl);if(!href||!sameHost(href,source.url)||seen.has(canonicalUrl(href)))continue;const text=stripHtml(m[2]).toLowerCase(),u=new URL(href),target=`${u.pathname}${u.search}`;const pagey=/(?:\/page\/\d+)|(?:[?&](?:page|paged|pg|p)=\d+)|(?:\/page-\d+)/i.test(target)||/^(?:next|older|more|\d+|›|»)$/.test(text);if(!pagey)continue;seen.add(canonicalUrl(href));out.push(canonicalUrl(href));}
  return out;
}

async function fetchText(url,accept="text/html,application/xhtml+xml;q=0.9,*/*;q=0.5"){
  const response=await fetch(url,{cache:"no-store",headers:{Accept:accept,"User-Agent":"RentalFinder26/2.2 (+rental discovery; respectful polling)","Cache-Control":"no-cache, no-store, max-age=0",Pragma:"no-cache"},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
...
function detailCandidate(html,url,source){
  ...
  const title=titleFromHtml(html)||"Rental listing",body=stripHtml(html).slice(0,35000);if(!/(rent|let|pcm|bed|apartment|flat|house|studio)/i.test(`${title} ${body}`))return null;
...
async function mapLimit(items,limit,fn){const out=new Array(items.length);let next=0;async function worker(){while(true){const i=next++;if(i>=items.length)return;try{out[i]=await fn(items[i],i);}catch(err){out[i]={__error:err};}}}await Promise.all(Array.from({length:Math.min(limit,items.length||1)},worker));return out;}
function statusFromError(source,err,started){const message=err?.message||"Source failed",status=err?.status;const kind=status===403||status===429?"blocked":message.includes("Timeout")||err?.name==="TimeoutError"?"timeout":"error";return{id:source.id,label:source.label,url:source.url,status:kind,items:0,indexPagesChecked:0,detailLinksFound:0,detailPagesChecked:0,error:message,elapsedMs:Date.now()-started,checkedAt:new Date().toISOString()};}

function seededStart(seed,length){
  if(length<=1)return 0;
  let h=2166136261;
  for(const ch of String(seed)){h^=ch.charCodeAt(0);h=Math.imul(h,16777619);}
  return Math.abs(h>>>0)%length;
}
function liveSelection(links,limit,scanId,sourceId){
  if(links.length<=limit)return links;
  const headCount=Math.min(3,limit),head=links.slice(0,headCount),rest=links.slice(headCount),need=limit-headCount;
  if(need<=0||!rest.length)return head;
  const start=seededStart(`${scanId}:${sourceId}`,rest.length),picked=[];
  for(let i=0;i<Math.min(need,rest.length);i++)picked.push(rest[(start+i)%rest.length]);
  return unique([...head,...picked],x=>x.url);
}

async function fetchPageSource(source,scanId){
  const started=Date.now();
  try{
    const firstPage=await fetchText(source.url),pages=[{url:source.url,text:firstPage.text}];
    ...
    const selected=liveSelection(allLinks,limit,scanId,source.id);
    ...
    const details=detailResults.filter(x=>x&&!x.__error),detailErrors=detailResults.filter(x=>x?.__error).length;
    ...
    return {items,status:{id:source.id,label:source.label,url:source.url,status:items.length?"working":"empty",items:items.length,indexPagesChecked:pages.length,detailLinksFound:allLinks.length,detailPagesChecked:selected.length-detailErrors,detailPagesFailed:detailErrors,scanMode:allLinks.length>selected.length?`live sample ${selected.length}/${allLinks.length}`:"full live scan",scanId,error:null,elapsedMs:Date.now()-started,checkedAt:new Date().toISOString()}};
  }catch(err){return{items:[],status:{...statusFromError(source,err,started),scanId}};}
}
async function fetchFeedSource(source,scanId){
  const started=Date.now();
  try{
    ...
    return {items,status:{id:source.id,label:source.label,url:source.url,status:items.length?"working":"empty",items:items.length,indexPagesChecked:1,detailLinksFound:items.length,detailPagesChecked:0,scanMode:"live feed",scanId,error:null,elapsedMs:Date.now()-started,checkedAt:new Date().toISOString()}};
  }catch(err){return{items:[],status:{...statusFromError(source,err,started),scanId}};}
}
...
export async function directSourceSearch(){
  ...
  const settled=await Promise.all(sources.map(source=>source.type==="feed"?fetchFeedSource(source,scanId):fetchPageSource(source,scanId)));
  const items=settled.flatMap(x=>x.items||[]),sourceStatuses=settled.map(x=>x.status);
  return {items,meta:{sourceStatuses,scanId}};
}
