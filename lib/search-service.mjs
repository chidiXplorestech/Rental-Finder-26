...
export async function refreshInventory(){
  ...
  return {snapshot,providerStats:discovery.providerStats,errors:discovery.errors,rawResults:discovery.raw.length,delta:snapshot.delta||null};
}
...
export async function runSearch(input={}){
  ...
  if(demo){
    ...
    return {items,meta:{provider:"Demo dataset",providers:[{id:"demo",label:"Demo dataset",calls:1,results:items.length,errors:[]}],rawResults:items.length,demo:true,persisted:false,partial:false,configuredSourceCount:directSourceStatuses().length,workingSourceCount:0,sourceStatuses:[],storage:storageMode(),cached:false,refreshedNow:false,delta:null},filters};
  }
  ...
  return {
    items,
    meta:{
      provider:"Tracked rental inventory",cached:!refreshMeta,refreshedNow:Boolean(refreshMeta),refreshedAt:snapshot?.refreshedAt||null,ageMinutes,
      ...
      providerErrors:refreshMeta?.errors||[],storage:storageMode(),delta:snapshot?.delta||null,
    },
    filters,
  };
}
